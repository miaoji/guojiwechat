export const SET_USERINFO = 'SET_USERINFO'

export const SET_SMSCODE = 'SET_SMSCODE'

export const SET_ADDRESS = 'SET_ADDRESS'

export const SET_ADDRESS_RES = 'SET_ADDRESS_RES'

export const SET_OPENID = 'SET_OPENID'

export const SET_ORDERLIST = 'SET_ORDERLIST'

export const SET_CARGOLIST = 'SET_CARGOLIST'

export const SET_TIPSHOW = 'SET_TIPSHOW'

export const SET_COUPON_LIST = 'SET_COUPON_LIST'

export const SET_COUPON_TOTAL_FEE = 'SET_COUPON_TOTAL_FEE'

export const SET_CAEGO_BUILD_LIST = 'SET_CAEGO_BUILD_LIST'

export const SET_CARGO_STATE = 'SET_CARGO_STATE'
